...
CREATE TABLE IF NOT EXISTS `proyectosfa`.`facturaimport` (
  ...
  `fecha_documento` DATE NULL,
  `fecha_creacion` DATE NULL,
  `numerotrasferencia` VARCHAR(50) NULL,
  `proveedor_id` INT NOT NULL,
  `banco` VARCHAR(30) NULL,
  `montotransferencia` DECIMAL NOT NULL,
  `nocuentat` VARCHAR(45) NOT NULL,
  `total` DECIMAL(11,2) NOT NULL,
  PRIMARY KEY (`idfactura`),
...
-- -----------------------------------------------------
-- Table `proyectosfa`.`aduaneroe`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `proyectosfa`.`aduaneroe` ;

CREATE TABLE IF NOT EXISTS `proyectosfa`.`aduaneroe` (
  `idaduaneroe` INT NOT NULL AUTO_INCREMENT,
  `fecha` DATE NULL,
  ...
  `total` DOUBLE NULL,
  `idvehiculo` INT NOT NULL,
  PRIMARY KEY (`idaduaneroe`),
  INDEX `fk_adu_proveedor_idx` (`idproveedor` ASC),
  INDEX `fk_adu_vehiculo_idx` (`idvehiculo` ASC),
  CONSTRAINT `fk_adu_proveedor`
    FOREIGN KEY (`idproveedor`)
    REFERENCES `proyectosfa`.`proveedor` (`idproveedor`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_adu_vehiculo`
    FOREIGN KEY (`idvehiculo`)
    REFERENCES `proyectosfa`.`vehiculo` (`idvehiculo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
